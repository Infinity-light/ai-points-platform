...
<template>
  <div class="p-6 max-w-4xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
      ...
      <h1 class="text-xl font-bold text-foreground">投票会议</h1>
    </div>
      ...
      <div class="bg-card border border-border rounded-lg p-5 mb-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <span
              ...
              :class="{
                'bg-blue-100 text-blue-700': session.status === 'open',
                'bg-green-100 text-green-700': session.status === 'passed',
                'bg-red-100 text-red-600': session.status === 'failed',
                'bg-gray-100 text-gray-600': session.status === 'closed',
              }"
            >
          ...
          <div v-if="session.status === 'open' && isHrOrAdmin">
            <BaseButton variant="outline" size="sm" :loading="closeLoading" @click="closeSession">
              关闭投票
            </BaseButton>
          </div>
        </div>
        ...
        <div v-if="votes.length > 0" class="mt-4 space-y-2">
          ...
          <div class="h-3 bg-muted rounded-full overflow-hidden flex">
            ...
            />
            <div class="h-full bg-red-400 flex-1" />
          </div>
          <p
        ...
        <div v-if="session.status === 'open'" class="mt-4">
          <div v-if="myVote !== null" class="text-sm text-muted-foreground">
            您已投票：
            <span :class="myVote ? 'text-green-600 font-medium' : 'text-red-500 font-medium'">
              {{ myVote ? '✓ 赞成' : '✗ 反对' }}
            </span>
          </div>
          <div v-else class="flex gap-3">
            <BaseButton class="flex-1" size="sm" :loading="voteLoading" @click="castVote(true)">
              ✓ 赞成
            </BaseButton>
            <BaseButton
              variant="outline"
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <div class="text-right text-xs">
              ...
              <div v-if="task.estimatedPoints" class="text-muted-foreground mt-1">
          ...
          >
            <div
              ...
              :key="key"
              class="bg-muted/50 rounded-md py-1.5"
            >
